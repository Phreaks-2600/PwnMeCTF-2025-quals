...
def new_launch_instance_action(
    ...
    def action() -> int:
        ...
        setup_addr, challenge_address = do_deploy(web3, deployer_account.address, player_account.address)
                ...
                json.dumps(
                    {
                        ...
                        "address": setup_addr,
                        "target_address": challenge_address
                    }
                )
            )
        ...
        print("it will automatically terminate in 60 minutes")
        print("here's some useful information :")
        print()
        print(f"UUID={uuid}")
        print(f"Casino URL=http://{PUBLIC_IP}:{HTTP_PORT}/{uuid}/")
        print(f"RPC=http://{PUBLIC_IP}:{HTTP_PORT}/{uuid}")
        print(f"PRIVATE_KEY={player_account.key.hex()}")
        print(f"PLAYER={player_account.address}")
        print(f"SETUP={setup_addr}")
        print(f"TARGET={challenge_address}")
        return 0
...
def new_get_flag_action(
    ...
    def action() -> int:
        ...
        print()
        print("Congratulations! You can now access vip page")
        return 0
